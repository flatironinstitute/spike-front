        ...
        if ((sr.studyName === study.name) && (sr.recordingName === rec.name)) {
          sorting_result_rows.push(
            <tr key={`sorting-result-${study.name}-${rec.name}-${sr.sorterName}`}>
              ...
              <td key={"output"}>{abbreviateSha1Path(sr.firings + '/firings.mda', { canDownload: false })}</td>
            </tr>
          )
        }
      }
              ...
              <div className="card card--stats">
                <div className="content">
                  <div className="card__footer">
                    ...
                    <ElectrodeGeometry locations={this.state.electrodeLocations} labels={electrodeLabels} />
                  </div>
                </div>
              </div>
            </Col>
