...
.tooltiptext {
  ...
  function PlotAreaSeries(x, y, props, opts) {
    let that = this;
    this.initialize = function(gg, plot_area) {
      initialize(gg, plot_area);
    };
    this.update = function() {
      update();
    };
    this.onMouseOver = function(callback) {
      m_handlers.mouseover.push(callback);
    };
    this.onMouseOut = function(callback) {
      m_handlers.mouseout.push(callback);
    };
    this.onClick = function(callback) {
      m_handlers.click.push(callback);
    };
    this.onMarkerMouseOver = function(callback) {
      m_handlers.marker_mouseover.push(callback);
    };
    this.onMarkerMouseOut = function(callback) {
      m_handlers.marker_mouseout.push(callback);
    };
    this.onMarkerClick = function(callback) {
      m_handlers.marker_click.push(callback);
    };
    this.onTooltipChanged = function(callback) {
      m_handlers.tooltip_changed.push(callback);
    };
    this.tooltip = function() {
      return clone(m_tooltip);
    };
    this.modifyOpts = function(opts) {
      modifyOpts(opts);
    };
    ...
    function handle_event(name, arg) {
      arg = arg || {};
      m_handlers[name].forEach(function(cb) {
        cb(arg);
      });
    }
    ...
    function update() {
      ...
      g.on("mouseover", function() {
        handle_event("mouseover", { mouse: d3.mouse(this) });
      })
        .on("mouseout", function() {
          handle_event("mouseout", { mouse: d3.mouse(this) });
        })
        .on("click", function() {
          handle_event("click", { mouse: d3.mouse(this) });
        });
      if (m_opts.lines) {
        let line = d3
          .line()
          .x(function(d) {
            return PP.xToPix(d.x);
          })
          .y(function(d) {
            return PP.yToPix(d.y);
          });
        let path = g.append("path").attr("d", line(data0));
        m_paths.push(path);
      }
      if (m_opts.markers) {
        data0.forEach(function(d, i) {
          let marker = g
            .append("circle")
            .attr("cx", PP.xToPix(d.x))
            .attr("cy", PP.yToPix(d.y));
          marker
            .on("mouseover", function() {
              handle_event("marker_mouseover", {
                index: i,
                mouse: d3.mouse(this)
              });
            })
  ...
  function example1(svg_id) {
    ...
    let Nth = 10;
    let theta = range(Nth + 1).map(function(v) {
      return (v * 2 * Math.PI) / Nth;
    });
    let x = theta.map(function(v) {
      return 20 * Math.cos(v);
    });
    let y = theta.map(function(v) {
      return 20 * Math.sin(v);
    });
    let props = theta.map(function(v, i) {
      if (i == 1)
        return { color: "blue", tooltip: "i=1", hover: { color: "red" } };
      else return {};
    });
    let opts = {
      marker: {
    ...
    function next() {
      ...
      */
      series.onMarkerClick(function(i) {
